...
import {  faCheckCircle, faInfo, faMapMarkerAlt, faPlaneDeparture, faTimesCircle, faUsers } from '@fortawesome/free-solid-svg-icons';
...
const TourPerPage = () => {
    ...
    useEffect(()=>{
        const url=`https://tour-express.herokuapp.com/offers/${id}`
fetch(url)
.then(res=>res.json())
...
e.preventDefault()
    ...
    const planeFont=<FontAwesomeIcon icon={faPlaneDeparture}></FontAwesomeIcon>
          ...
          <div className="w-full">
           <img src={photo} alt="" className="w-full rounded-md  lg:h-96 " />
            ...
            <div className="lg:flex justify-between p-4">
                ...
                <h4 className="text-gray-500"><span className="text-red-500">{mapFont}</span> {location}</h4>
                </div>
            ...
            <div className="lg:flex space-y-4 lg:space-y-0 justify-between border-t-2 border-b-2 border-gray-500 p-4">
                ...
                <div className="flex justify-center items-center gap-4">
                   <span  className="text-4xl text-pink-500"> {planeFont}</span>
                    <div>
                        <p className="font-bold">Tour Type</p>
                        <small className="text-blue-900 font-medium">{tourType}</small>
                    </div>
                </div>
                <div className="flex justify-center items-center gap-4">
            ...
            <div className=" space-y-4">
                ...
                <table className="w-full text-left rounded-xl border-2 ">
                 ...
                 <tr className="border-2">
                        <td className="border-2 font-medium text-indigo-900 p-4">Destination</td>
                        <td className="border-2 p-4">{location}</td>
                    </tr>
                    <tr className="border-2">
                        <td className="border-2 font-medium text-indigo-900 p-4">Depurture time</td>
                        <td className="border-2 p-4">{departureTime}</td>
                    </tr>
                    <tr className="border-2">
                        <td className="border-2 font-medium text-indigo-900 p-4">Ruturn Time</td>
                        <td className="border-2 p-4">{returnTime}</td>
                    </tr>
                    <tr className="border-2">
